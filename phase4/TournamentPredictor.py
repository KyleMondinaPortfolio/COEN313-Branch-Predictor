...
class TournamentPredictor:
    ...
    def update(self,actual_value):
        sr_correct = actual_value == self.shift_register.predict()
        bp_correct = actual_value == self.bit_predictor.predict()
        
        #update the two predictors
        self.shift_register.update(actual_value)
        self.bit_predictor.update(actual_value)

        if (sr_correct == bp_correct):
            #do nothing
            return
        if (sr_correct and (not bp_correct)):
            self.skew_to_SR()
            return
        if ((not sr_correct) and bp_correct):
            self.skew_to_BP()
            return
